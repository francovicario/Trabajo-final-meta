...
El test de Begg da un resultado marginal (z = 1.79, p = 0.074).
Esto sugiere una indicación débil de asimetría, pero no alcanza(p< .05).
Pero el test de Begg tiene baja potencia con k bajo (ejemplo 19 estudios), un p cercano a 0.05 puede interpretarse como una señal pero no como evidencia concluyente.
...
MOstrando una tendencia a la asimetría, coherente con lo visto en el funnel plot, pero que aún no es concluyente.
...
Este test es el más robusto cuando existe heterogeneidad, este si resulta estadísticamente significativo (t = 2.53, p = 0.0218).
Dado que este método ajusta la heterogeneidad de manera más apropiada, se considera un método más confiable bajo condiciones de nuestro metaanálisis.
La significación en este reporta asimetría del funnel plot, evidenciando de manera más sólida el sesgo de publicación.
